...
class APIClient: APIClientProtocol {
    ...
    init() throws {
        ...
        self.client = Client(
            ...
            transport: AsyncHTTPClientTransport(
                configuration: .init(client: httpClient, timeout: .minutes(1)))
        )
        ...
        -> Components.Schemas.XcProjectPublic
    {
        try await handleRequestError {
            ...
            case .badRequest(let response):
                throw APIError.clientRequestError(
                    statusCode: 400, detail: try response.body.json.detail)
            case .unprocessableContent(let response):
                throw APIError.clientRequestError(
                    statusCode: 422, detail: try response.body.json.detail.description)
            case .internalServerError:
                throw APIError.serverError(statusCode: 500)
            case .undocumented(let statusCode, _):
                throw APIError.unknownStatus(statusCode: statusCode)
    ...
    func listBuilds(projectId: String) async throws -> [Components.Schemas.BuildPublic] {
        try await handleRequestError {
            let result = try await client.projectsListBuilds(
    ...
    {
        try await handleRequestError {
            let result = try await client.projectsStartBuild(
                .init(path: .init(projectId: projectId), body: .json(data)))
            ...
            case .notFound(let response):
                throw APIError.clientRequestError(
                    statusCode: 404, detail: try response.body.json.detail)
            case .unprocessableContent(let response):
                throw APIError.clientRequestError(
                    statusCode: 422, detail: try response.body.json.detail.description)
            case .badRequest(let response):
    ...
    > {
        try await handleRequestError {
            let result = try await client.projectsStreamBuildUpdates(
                .init(path: .init(projectId: projectId, buildId: buildId)))

            switch result {
            case .ok(let okResponse):
                let stream = try okResponse.body.textEventStream
    ...
    func listAvailableTests(projectId: String, buildId: String) async throws -> [String] {
        try await handleRequestError {
            let result = try await client.projectsListAvailableTests(
                .init(path: .init(projectId: projectId, buildId: buildId)))
            switch result {
    ...
    func enableDeveloperMode(deviceId: String) async throws {
        try await handleRequestError {
            let result = try await client.devicesEnableDeveloperMode(
                .init(path: .init(deviceId: deviceId)))
            switch result {
            case .ok:
                return
            case .internalServerError:
    ...
    func connectTunnel(deviceId: String) async throws {
        try await handleRequestError {
            let result = try await client.devicesConnectTunnel(
                .init(path: .init(deviceId: deviceId)))
            switch result {
            case .ok:
                return
    ...
    {
        try await handleRequestError {
            let result = try await client.testPlansUpdateTestPlan(
                path: .init(testPlanId: testPlanId), body: .json(data))
    ...
    func deleteTestPlan(testPlanId: String) async throws {
        try await handleRequestError {
            let result = try await client.testPlansDeleteTestPlan(
                path: .init(testPlanId: testPlanId))
    ...
    {
        try await handleRequestError {
            let result = try await client.testPlansCreateTestPlanStep(
                path: .init(testPlanId: testPlanId), body: .json(data))
    ...
    func updateTestPlanStep(
        ...
        try await handleRequestError {
            let result = try await client.testPlansUpdateTestPlanStep(
                path: .init(testPlanId: testPlanId, stepId: stepId), body: .json(data))
    ...
    func deleteTestPlanStep(testPlanId: String, stepId: String) async throws {
        try await handleRequestError {
            let result = try await client.testPlansDeleteTestPlanStep(
                path: .init(testPlanId: testPlanId, stepId: stepId))
    ...
    func reorderTestPlanSteps(testPlanId: String, ids: [String]) async throws {
        try await handleRequestError {
            let result = try await client.testPlansReorderTestPlanSteps(
                path: .init(testPlanId: testPlanId), body: .json(ids))
    ...
    func listTestSession(projectId: String) async throws -> [Components.Schemas.TestSessionPublic] {
        try await handleRequestError {
            let result = try await client.testSessionListTestSessions(
                query: .init(projectId: projectId))
    ...
    func cancelTestSession(sessionId: String) async throws {
        try await handleRequestError {
            let result = try await client.testSessionCancelTestSession(
                path: .init(testSessionId: sessionId))
    ...
    func streamSessionExecutionStepUpdates(sessionId: String) async throws -> AsyncThrowingStream<
        Components.Schemas.ExecutionStepPublic, any Error
    > {
        try await handleRequestError {
            let result = try await client.testSessionStreamExecutionStepUpdates(
                path: .init(testSessionId: sessionId))
                ...
                return AsyncThrowingStream { continuation in
                    let task = Task {
                        do {
                            ...
                            logger.warning(
                                "Finish listening to execution step updates with error \(error)")
    ...
    func exportSessionResults(sessionId: String) async throws {
        try await handleRequestError {
            let result = try await client.testSessionExportTestSessionResults(
                path: .init(testSessionId: sessionId))
