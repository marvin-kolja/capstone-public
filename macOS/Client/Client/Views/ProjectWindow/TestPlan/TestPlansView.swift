...
struct TestPlansView: View {
    ...
    var body: some View {
        TwoColumnView(
            content: {
                LoadingView(
                    isLoading: testPlanStore.loadingTestPlans,
                    hasData: !testPlanStore.testPlans.isEmpty,
                    refresh: {
                        Task {
                    ...
                    ZStack {
                        List(testPlanStore.testPlans, id: \.id, selection: $selectedTestPlanId) {
                            testPlan in
                            HStack {
                                Text(testPlan.name)
                                Spacer()
                                LoadingButton(
                                    isLoading: testPlanStore.deletingTestPlans[testPlan.id] ?? false
                                ) {
                                    Task {
            ...
            detail: {
                if let testPlanId = selectedTestPlanId,
                    let testPlan = testPlanStore.getTestPlanById(testPlanId: testPlanId)
                {
                    TestPlanDetailView(testPlan: testPlan)
                        ...
                        .environmentObject(
                            TestPlanStepStore(
                                projectId: testPlanStore.projectId, testPlanId: testPlan.id,
                                apiClient: testPlanStore.apiClient, steps: testPlan.steps))
                } else {
