        ...
        Section(isExpanded: $isExpanded) {
            VStack(alignment: .center) {
                ...
                if let traceResult = traceResult {
                    ...
                    case .cpu:
                        PerformanceChart(
                            data: traceResult.cpuChartData, yFormat: .percent, yTitel: "CPU",
                            markType: .line)
                    case .gpu:
                        PerformanceChart(
                            data: traceResult.gpuChartData, yFormat: .percent, yTitel: "GPU",
                            markType: .line)
                    case .fps:
                        PerformanceChart(
                            data: traceResult.fpsChartData, yFormat: .fps, yTitel: "FPS",
                            markType: .bar)
                    case .memory:
                        PerformanceChart(
                            data: traceResult.memoryChartData, yFormat: .mb, yTitel: "Memory",
                            markType: .line)
                    }
                } else {
                    Text("No Data")
