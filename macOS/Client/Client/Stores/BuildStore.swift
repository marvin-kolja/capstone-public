...
class BuildStore: ProjectContext {
            ...
            if let apiError = appError.type as? APIError {
                ...
                case .clientRequestError:
                    errorLoadingBuilds = AppError(
                        type: LoadBuildsError.invalidProjectId(projectId: projectId))
                default:
                    ()
                }
            }
    ...
    func streamUpdates(buildId: String) async {
        ...
        do {
            let stream = try await apiClient.streamBuildUpdates(
            ...
            if let apiError = appError.type as? APIError {
                ...
                case .clientRequestError:
                    errorStreamingBuildsUpdates[buildId] = AppError(
                        type: StreamBuildUpdatesError.invalidProjectId(projectId: projectId))
                default:
                    ()
                }
            }
            ...
            errorStreamingBuildsUpdates[buildId] = AppError(
                type: StreamBuildUpdatesError.unexpected)
        }
    }
    ...
    func startBuild(buildId: String) async {
        ...
        do {
            ...
            let requestData = Components.Schemas.StartBuildRequest(
                configuration: build.configuration, deviceId: build.deviceId, scheme: build.scheme,
                testPlan: build.testPlan)
            let newBuild = try await BuildStore.startBuild(
                projectId: projectId, data: requestData, apiClient: apiClient)
            setBuild(build: newBuild)
        } catch {
    ...
    static func startBuild(
        projectId: String, data: Components.Schemas.StartBuildRequest, apiClient: APIClientProtocol
    ) async throws -> Components.Schemas.BuildPublic {
        do {
    ...
    func loadAvailableTests(buildId: String) async {
        ...
        do {
            let xcTestCases = try await apiClient.listAvailableTests(
                projectId: projectId, buildId: buildId)
            ...
            if let apiError = appError.type as? APIError {
                ...
                case .clientRequestError:
                    errorsListingAvailableTests[buildId] = AppError(
                        type: ListAvailableTestsError.unexpected)
                default:
                    ()
                }
            }
