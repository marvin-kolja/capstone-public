...
| [`xcrun xcresulttool`](https://developer.apple.com/documentation/xcode/xcresulttool)              | Command line tool used to exatract data from `.xcresult` files.   |
...
The core library uses `pytest` for testing:
...
- `--device` - Run tests that interact with a physical iOS device.
